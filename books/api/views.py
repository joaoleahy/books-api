...
class BookViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiExample(
                "Success", value={"status": "Data updated successfully"}
            ),
            400: OpenApiExample("Error", value={"error": "Could not update data"}),
        },
    )
    ...
    def refresh_enriched_data(self, request: Any, pk: Any = None) -> Response:
        ...
        return Response(
            {"error": "Could not update data"}, status=status.HTTP_400_BAD_REQUEST
        )
