...
class BookAdmin(admin.ModelAdmin):
    ...
    def display_cover_thumbnail(self, obj):
        ...
        if obj.enriched_data and obj.enriched_data.get("image_links", {}).get(
            "thumbnail"
        ):
            return format_html(
                '<img src="{}" height="50"/>',
    ...
    def display_enriched_info(self, obj):
        ...
        if obj.enriched_data.get("authors"):
            info.append(
                f"<strong>Authors:</strong> {', '.join(obj.enriched_data['authors'])}"
            )
        ...
        if obj.enriched_data.get("published_date"):
            info.append(
                f"<strong>Publication Date:</strong> {obj.enriched_data['published_date']}"
            )

        if obj.enriched_data.get("page_count"):
            info.append(
                f"<strong>Page Count:</strong> {obj.enriched_data['page_count']}"
            )

        if obj.enriched_data.get("categories"):
            info.append(
                f"<strong>Categories:</strong> {', '.join(obj.enriched_data['categories'])}"
            )
        ...
        if obj.enriched_data.get("average_rating"):
            ...
            info.append(
                f"<strong>Rating:</strong> {stars} ({obj.enriched_data['average_rating']})"
            )
            if obj.enriched_data.get("ratings_count"):
                info.append(
                    f"<strong>Number of Ratings:</strong> {obj.enriched_data['ratings_count']}"
                )

        # Add description if available
        if obj.enriched_data.get("description"):
            info.append(
                f"<strong>Description:</strong><br>{obj.enriched_data['description']}"
            )
